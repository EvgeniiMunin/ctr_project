...
from src.features.build_transformer import build_transformer, process_count_features, build_ctr_transformer, \
    extract_target
...
def test_train_model(
    ...
    dataset['hour'] = dataset.hour.apply(lambda val: datetime.strptime(str(val), '%y%m%d%H'))
    ...
    model = train_model(
        train_features, train_target, TrainingParams()
    )
